...
namespace Therapim.Controllers
{
    ...
    public class ReviewController : Controller
    {
        ...
        /// レビュー一覧画面表示
        /// </summary>
        /// <returns>レビュー一覧画面 with <ReviewResponseModel></returns>
        [HttpGet]
        public async Task<IActionResult> List()
        {
            //取得処理
            var reviewProcesser = new ReviewProccesser(_logger, _commonService);
            var reviewList = await reviewProcesser.getReviewList();

            if(reviewList == null)
            {
                // TempData にエラーメッセージを設定
                TempData["ErrorMessage"] = "レビュー情報取得の通信に失敗しました。再度お試しいただいても解決しない場合は店舗までお問い合わせください。";
                // エラー画面にリダイレクト
                return RedirectToAction("Error", "Home");
            }
            //一覧画面で、ユーザIDが一致するレビューには編集ボタンを表示するためのパラメータを持たせる
            ViewData["UserID"] = _commonService.GetCookieUserId() == null? "": _commonService.GetCookieUserId();            
